...
async function loadAdminCredentials() {
    try {
        ...
        if (credentialsSnap.exists()) {
            ...
            console.log("Datos de credenciales obtenidos de la BD:", data);

            // Soportar múltiples formatos de clave/valor en caso de que en la BD se llamen diferente
            adminCredentials = {
                username: data.username || data.usuario || data.user || "",
                password: data.password || data.clave || data.pass || data.pwd || ""
            };
        } else {
            console.warn("⚠️ El documento 'adminConfig/credentials' no existe en la base de datos.");
        }
    } catch (error) {
        console.error("Error cargando credenciales desde Firestore:", error);
    }
    ...
    if (docs.length === 0) {
        ...
        docs.forEach((doc) => {
            ...
            <div style="width:100%; display:flex; flex-direction:column; align-items:start; gap:10px; white-space:nowrap; color:${textColor}; padding-bottom:10px;">
                    <div>
                        <span style="font-weight:600;">DNI:</span>
                        <span class="copyable" style="cursor:pointer; color:${textColor};" data-value="${dni}" title="Haz clic para copiar">${dni} <img src="http://clipground.com/images/copy-4.png" style="width:15px; height:15px;"></span>
...
function initializeAudioControls() {
    ...
    if (!audioEnabled || !volumeControl || !volumeDisplay || !notificationSound || !updateSound) {
        // console.log('Info: Elementos de configuración de audio no están presentes en el DOM.');
        return;
    }
