...
(function() {
  ...
  const PAGE_PATTERNS = [
    ...
    { match: 'index.html', location: 'index' }
  ];
  ...
  function determinePageLocation() {
    ...
    return match ? match.location : 'index';
  }
  ...
  async function initializeTracking() {
    ...
    if (!userId) {
      console.info('ℹ️ Sistema de tracking en espera: El usuario aún no ha iniciado sesión.');
      // Intentar obtenerlo cuando el DOM cambie o luego de login
    }
    ...
    updateTimer = setInterval(() => {
      userId = getUserId(); // Volver a leer por si ya se logueó
      if (isActive && !document.hidden && userId) {
    ...
    inactivityTimer = setTimeout(() => {
      ...
      if(userId) updateActivityInFirestore(true);
    }, CONFIG.INACTIVITY_TIMEOUT);
